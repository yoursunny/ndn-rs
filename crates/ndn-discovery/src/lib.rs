...
pub use config::{
    DiscoveryConfig, DiscoveryProfile, DiscoveryScope, HelloStrategyKind,
    PrefixAnnouncementMode, ServiceDiscoveryConfig, ServiceValidationPolicy,
};
...
pub use probe::{
    DirectProbe, IndirectProbe,
    build_direct_probe, build_indirect_probe, build_indirect_probe_encoded,
    build_probe_ack, is_probe_ack, parse_direct_probe, parse_indirect_probe,
};
...
pub use scope::{
    ...
    mgmt_prefix, ndn_local, nd_root, peers_prefix, probe_direct, probe_via,
    routing_lsa, routing_prefix, scope_root, sd_services, sd_updates, site_root,
};
pub use strategy::{
    build_strategy, BackoffScheduler, NeighborProbeStrategy, PassiveScheduler,
    ProbeRequest, ReactiveScheduler, SwimScheduler, TriggerEvent,
};
